...
class WRN_50_2(nn.Cell):
    def __init__(self, num_classes=2388):
        ...
        self.conv1 = nn.Conv2d(
            3, 64, kernel_size=7, stride=2, pad_mode="pad", padding=3
        )
        ...
        self.layer1 = self._make_layer(
            init_block_channels, channels_per_layer[0], k, block_nums[0], stride=1
        )

        # 第二个卷积组，输出通道数为128×k，步长为2
        self.layer2 = self._make_layer(
            channels_per_layer[0], channels_per_layer[1], k, block_nums[1], stride=2
        )

        # 第三个卷积组，输出通道数为256×k，步长为2
        self.layer3 = self._make_layer(
            channels_per_layer[1], channels_per_layer[2], k, block_nums[2], stride=2
        )

        # 第四个卷积组，输出通道数为512×k，步长为2
        self.layer4 = self._make_layer(
            channels_per_layer[2], channels_per_layer[3], k, block_nums[3], stride=2
        )
